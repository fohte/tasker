...
export const taskQueries = {
  ...
  searchTasks: async (searchTerm: string) => {
    return await db.select().from(tasks).where(like(tasks.title, `%${searchTerm}%`));
  },
...
export const labelQueries = {
  ...
  getLabelsByTaskId: async (taskId: string) => {
    ...
    const labelsData = await Promise.all(
      labelIds.map(id => db.select().from(labels).where(eq(labels.id, id)).get())
    );
...
export const taskLinkQueries = {
  ...
  getParentTask: async (childId: string) => {
    ...
    return await db.select().from(tasks).where(eq(tasks.id, parentLink.parentId)).get();
  },
  
  // タスクリンク作成（親子関係の作成）
  createTaskLink: async (parentId: string, childId: string, relation: string = 'subtask') => {
